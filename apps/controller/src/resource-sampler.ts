...
export class ResourceSampler {
  ...
  private beginSample(): Promise<void> {
    ...
    const samplePromise = this.collectSample()
      .catch(() => {
        this.sampleIncomplete = true;
      })
      .finally(() => {
        if (this.samplePromise === samplePromise) this.samplePromise = undefined;
      });
  ...
  private async collectSample(): Promise<void> {
    ...
    if (pids === undefined) throw new Error("resource sampler PIDs are unavailable");
