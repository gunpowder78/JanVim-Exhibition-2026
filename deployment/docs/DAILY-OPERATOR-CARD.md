...
JanVim 启动后自动原生最大化；三屏启动确认后直接隐藏任务栏，正常退出后恢复。
开机展示启动期间无需点击 JanVim；冷启动验收时请先不操作鼠标，确认任务栏消失。
若异常结束后任务栏未恢复，双击桌面“恢复任务栏”；详细命令见 `TASKBAR-RECOVERY.md`。
搬机或改动 Windows 显示布局后仍需检查实际屏序。
