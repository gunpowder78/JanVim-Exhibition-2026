...
现场维护版启动器会按 Windows 本次开机时间识别上一启动周期遗留的有效运行标记，
将其原样移入新诊断目录并继续自动开演。当前启动周期或不一致的记录保持拒绝；不读取旧
声音会话，不覆盖人工映射。整个启动及展示期间持有机器级互斥锁，防止多入口重复启动。
...
5. B 屏 `STOP SHOW` 只关闭展示；`Ctrl+Shift+S` 在完整退出展示、声音淡出及服务清理成功后关闭小主机。B 屏出现黄色退出提示即表示请求已受理，请等待当前循环到复位边界，不要重复操作。
...
- `jianshan-placement-1-Stdout.txt` / `Stderr.txt`：窗口定位结果与具体错误，每条输出最多 32,768 字符；有重试时另存第 2、3 次结果。

校验通过后终端继续提示屏幕匹配、声音启动、窗口定位及 JanVim / Narrative 启动阶段。
《见山》窗口尚未出现、最终位置暂未达到要求或定位辅助进程超时，仅对同一个 PID 与创建时间
最多定位三次，间隔 500 毫秒。每次辅助进程上限 20 秒（强制收尾另限 2 秒）；不会重新开启
声音服务或另一个《见山》。进程身份不符、多窗口歧义和未知错误不重试。
...
显示设备，且处于扩展模式（桌面区域不重叠）。完整桌面几何唯一且与人工配置一致时，优先
按保存的位置、分辨率、缩放及旋转恢复角色，即使 Windows 重分配或复用了设备 ID。
几何不适用而三个 ID 都仍存在时，沿用其角色分配；显示名称变化不影响匹配。
...
2026-09-09 现场维护：用户确认画面、声音、屏序及 Ctrl+Shift+S 退出后关机正常。随后冷启动
校验用时 95.122 秒并通过，失败点为《见山》定位；旧脚本没有保存该辅助程序的具体错误。
当日再次手动触发同一任务，约 16 秒内《见山》已在正确屏幕全屏、展演运行。该暖启动结果
不能证明冷启动修复。新增有限重试及诊断后仍需真实关机、开机、登录后的自动开演验收。
