...
- [x] Write WAV analysis tests first with handcrafted PCM16 stereo RIFF bytes: reject truncated/misaligned/unsupported/nonfinite data, calculate independent exact peak/RMS/segment silence. Export `analyzeWav(buffer, segments)` with format, duration, channels, per-segment peak/RMS; no whole-file unbounded input (>64MiB reject).
- [x] Write a real NRT render test before definitions exist. Use Score with production definitions and include isolated pluck/wind/mix and stop sections; final dummy event leaves >=2 seconds of postfade silence. Render at 48k PCM16 and no device boot. Assertions: initial/postfade peak <=1/32768, active peak >0.01 and <=0.2001, both channels nonzero, no clipped samples. This is signal validation, not aesthetic approval.
- [x] Run RED, implement definitions and service. Voices have finite lifecycle and at most eight plucks; one wind. Smooth wind changes; limiter precedes fade. Clamp voice inputs at synthesis boundary too. Stop latches policy, fades all via mixer; ignores later activity.
- [x] Implement mixer DSP lease with changing heartbeat control, a 2 s timer and latched 1.5 s envelope release; accepted heartbeat updates it. Test missing DSP heartbeat independently in NRT, so language-process death cannot leave the wind audible. Silent service must omit audible output Synth rather than multiply a potentially invalid signal by zero.
- [x] Integrate policy `handle`/`tick` in OSCdef (loopback only); 50ms timeout checker. Bound startup <=30s, cleanup <=8s, duration <=3600s. Fail closed to silence on SC /fail or server exit; no unbounded recovery. Quit only this own server. Write capture then free nodes/buffers after bounded fade. No raw OSC passthrough.
- [x] Include the newly booted `server.pid` as `serverPid` in READY; the supervisor validates and pins live identity/ancestry in memory before language-only interruption or orphan cleanup. Never authorize process termination from a disk receipt alone.
- [x] Avoid the stock `Server.boot` auto-reclaim branch (`prPingApp` can call quit on an existing responder). Use a minimal owned-process startup path and an occupied-responder regression fixture; never rely on a released preflight bind to authorize later automatic reclamation.
- [x] Run silent NRT checks and language compile/policy checks; report tests. Local commit/review.

Evidence: implementation `4e95208`, fixes `2f394e9` / `ce3e2ae`; 14/14 WAV/service checks, 8/8 OSC checks, SC policy/startup isolation/runner checks; independent task review and both scoped fix reviews complete with no open findings.
...
Review correction: permit bounded calls to the existing service statistics emitter at stop/final-cleanup boundaries, if required to observe accepted counters before/after the real post-stop UDP probe. Periodic statistics stop during cleanup. This adds no raw control gateway or policy/admission change.
...
- [x] Implement sender timeline deterministically: plucks-only first third, wind-only second, mixed last third; x/y/motion/energy vary with fixed formula. Heartbeat every .25s; no timer catch-up audio burst after stall. Send start once and await event acknowledgment. Stop packets at most three attempts over <=.3s with increasing seq; supervisor then awaits fade/cleanup. SIGINT/SIGTERM call same idempotent stop. Timeout kills only the ChildProcess created by this invocation and its exact descendants; never processes by name. No automatic audible restart.
- [x] Implement silent integration recording using actual UDP -> policy -> SynthDefs -> capture: nonzero sections, correct stop silence; send stale/wrong-session/extra-argument/NaN and poststop activity probes. Verify no accepted activity after stop. Kill only the newly created sender/sclang in interruption tests; assert port reuse and fresh launch. Do not record continuously on 30min soak.
- [x] Distinguish whole-tree shutdown from sclang-only loss. Pin this launch's server identity while both are alive, verify independent DSP timeout behavior, then reclaim only the same owned audio process if it is orphaned.
- [x] Run tests and silent demo, then 30min silent resource test with sampled own child working sets and node counts. One 10-hour fake-clock policy run remains separately labeled. Local commit/review.

## Task 4: Whole-branch verification, read-only integration notes and handoff

Task 3 implementation/review complete: `3b050ba`, fix `dd83918`; 37/37 combined sound checks, 3 SC checks, lint and scoped re-review clean. First 30-minute run is explicitly pre-lint/pre-review-fix evidence; delivery's exact-source long validation remains coordinated in Task 4 after whole-branch review.
...
- [ ] Resolve N1 with a failing zero-status lifetime regression, the minimal READY/COMPLETE-based grace-condition correction, and focused verification before claiming all abnormal exits fade or proceeding to formal listening acceptance. Final review cap reached; no silent waiver or second fix wave in this run.
